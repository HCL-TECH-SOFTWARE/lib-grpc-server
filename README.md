...
This sample contains a [gRPC server](/blob/main/grpc-server) (implemented with Model RealTime) and a [gRPC client]((/blob/main/grpc-client)) (implemented as a stand-alone C++ command-line application).
...
## Build the client
3. You need to build the client before the server since it contains the [.proto file](/blob/main/grpc-client/proto/maze.proto) that describes the RPCs implemented by the sample. This file is used both by the client and the server.
...
## Build the server
4. Open the [gRPC server](/blob/main/grpc-server) project in Model RealTime.
5. Open the TC `server.tcjs` and edit the variables in the beginning of the file that specify the location where you placed the gRPC source code (`grpcSourceLocation`) and where you installed the gRPC tools and libraries (`grpcInstallLocation`).
...
* **subscribe** Subscribe to get notified about an attempt to take a step in a direction that is not possible.
...
Refer to the [.proto file](/blob/main/grpc-client/proto/maze.proto) and the picture below to understand how the sample works:
...
In the sample client, the finishing of a subscription request will terminate the subscription thread.